...
<html>
<head>
<title>Spring 3 MVC Series - documenttype Manager</title>
...
<script>	
		$(document).ready(function() {
			$("#documenttypefrm").validationEngine()
		});
	...
	function documenttypeDelete() {
		var checkFound = false;
		var conf = "";
		conf = confirm("Are you sure,you want to delete this DocumentType?")
		if (conf == true) {
			document.documenttypefrm.action = "<%=request.getContextPath()%>/documenttype/deletedocumenttype/"+ document.documenttypefrm.id.value;
			document.documenttypefrm.submit();
		}
	}
	 ...
	 */
	function documenttypeCancel() {
			document.documenttypefrm.method=""
			document.documenttypefrm.action = "<%=request.getContextPath()%>/documenttype/documenttypelist";
			document.documenttypefrm.submit();
	...
	<div id="outer">
		<div id="outer2">
			...
			<div id="content">
				<div id="column1">
					<h3>Site</h3>
					<form:form method="post" action="${action}" commandName="documenttype" name="documenttypefrm" id="documenttypefrm" class="formular">
							...
							<tr>
								<c:choose>
									...
									<c:otherwise>
										...
										<td>
										<input class="styled-button-1" type="submit"  value="Delete" onclick="documenttypeDelete()" />
										</td>
										<td><input type="button" class="styled-button-1" value="Cancel" onclick="documenttypeCancel()"></td>
										<td>
											<input class="styled-button-1" type="submit"  value="Close all prompt" onclick="$.validationEngine.closePrompt('.formError',true)" />
										</td>
									</c:otherwise>
